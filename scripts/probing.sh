...
cat ~/recon/$1/Subdomains/$1_subdomains.txt | httpx -follow-redirects -status-code -vhost -title -threads 300 -silent | sort -u >> $results/$1_alive_with_status.txt;
cat ~/recon/$1/Subdomains/$1_subdomains.txt | httpx -follow-redirects -status-code -vhost -threads 300 -silent | sort -u | grep "[200]" | cut -d [ -f1 | sort -u | sed 's/[[:blank:]]"$//' >> $results/$1_fine_alive_urls.txt;
